...
| [types.md](types.md) | Built-in scalars, pointer MVP `*T`, fixed arrays `[T; N]` including nested arrays, typing rules. | Arrays are accepted for locals, function parameters/returns, and module `const`; pointer MVP is local scalar only. |
| [expressions.md](expressions.md) | Literals, operators, calls, address/deref, array literals, indexing `a[i]` / `a[i][j]`, precedence. | Logical `&&` / `\|\|` short-circuit; module `const` lowers `&&`/`\|\|` eagerly as truthified `Binary` for a simpler initializer IR. |
